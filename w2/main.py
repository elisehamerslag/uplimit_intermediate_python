...
def batch_files(file_paths: List[str], n_processes: int) -> List[set]:
    ...
    for ind, each_file in enumerate(second_set):
        # add to the list, add instead of append because it's a set.
...
def main() -> List[Dict]:
    ...
    with multiprocessing.Pool(processes=n_processes) as pool:
        params = [(file_paths, id_process) for id_process, file_paths in enumerate(batches)]
        revenue_data = pool.starmap(run, params) # starmap submits tasks for that pool. (The task is 'run')
        revenue_data = flatten(revenue_data) # flatten is necessary because the starmap function will return a list.
        # pool.close()  # optional
        # pool.join()  # optional
        print(revenue_data)
